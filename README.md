...
* `ssh_keypair_name`: Name of the SSH key pair to register with nova (Default:
  none)
* `gitlab_admin_user`: The admin user login id (Default: root)
* `kitchen`: URL for the kitchen to clone with git. The Chef Solo run will copy
  ...
  https://github.com/rackspace-orchestration-templates/gitlab)
...
Stack Details
=============
Gitlab v6.7.3 is installed into /srv/git. A self-signed SSL certificate is
generated and installed. Access to port 80 will be redirected to port
443. The kernel firewall is not enabled, but rules are configured to allow
access on ports 80, 443, & 22.
